...
# setup php-fpm
if [[ ${TRAVIS_PHP_VERSION:0:1} = "7" ]] || [[ ${TRAVIS_PHP_VERSION:0:1} = "8" ]]; then sudo cp config/Shared/ci/travis/www.conf ~/.phpenv/versions/$(phpenv version-name)/etc/php-fpm.d/www.conf; fi
